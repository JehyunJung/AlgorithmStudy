def solution(space,xpos,ypos):
    ...
    current_xside=(0,2**space)
    current_yside=(0,2**space)
    count=0
    while True:
        ...
        if ypos >= med_y and ypos <current_yside[1]:
            count+=space_count//2
            current_yside=(med_y,current_yside[1])
        else:
            current_yside=(current_yside[0],med_y)

        if xpos > med_x and xpos <=current_xside[1]:
            count+=space_count//4
            current_xside=(med_x,current_xside[1])
        else:
            current_xside=(current_xside[0],med_x)

        if current_yside[0]==ypos and current_xside[1]==xpos:
            count+=1
            break
        space_count//=4
